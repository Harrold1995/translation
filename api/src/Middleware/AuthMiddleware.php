...
class AuthMiddleware
{
    ...
    public function __invoke(Request $request, Response $response, $next): ResponseInterface
    {
        ...
        if (empty($jwtHeader) === true) {
            throw new \ErrorException('JWT Token required.', 400);
        }
        $jwt = explode('Bearer ', $jwtHeader);
        if (!isset($jwt[1])) {
            throw new \ErrorException('JWT Token invalid.', 400);
        }
    ...
    public function checkToken(string $token)
    {
        try {
            ...
            throw new \ErrorException(self::FORBIDDEN_MESSAGE_EXCEPTION, 403);
