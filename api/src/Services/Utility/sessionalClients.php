...
class sessionalClients extends standardSqlRequest
{
	...
	public static function getClients($roleID) {
        ...
        $s->where( 'role_id', $staffRoleID, '=', 'roles') ;
        $s->select() ;
        //echo $s->sql;
        */

        $queryString = "SELECT c.`Client ID` AS new_client_id FROM role_clients AS a, roles AS b, clients AS c WHERE b.role_id = $roleID AND a.role_id = b.role_id AND c.id = a.client_id";
